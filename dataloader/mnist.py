...
import torch
import codecs
from .utils import noisify,noisify_instance_dependent
        ...
        if self.train:
            ...
            if noise_type != 'clean':
                if noise_type != 'instance':
                    ...
                    self.train_noisy_labels, noise_rate = noisify_instance_dependent(self.train_data.numpy(),self.train_labels,noise_rate = noise_rate,img_size=28*28*1)
                    self.transition_matrix = None
                    self.noise_or_not = np.transpose(self.train_noisy_labels)==np.transpose(self.train_labels)
                    print(self.noise_or_not)
                    print('actual noise',noise_rate)
        else:
