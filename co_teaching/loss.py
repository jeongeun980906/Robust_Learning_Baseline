...
def loss_coteaching(y_1, y_2, t, forget_rate, ind, noise_or_not):
    ...
    if len(ind_1_update) == 0:
        ind_1_update = ind_1_sorted.cpu()
        ind_2_update = ind_2_sorted.cpu()
...
def loss_coteaching_plus(logits, logits2, labels, forget_rate, ind, noise_or_not, step):
    ...
    outputs2 = F.softmax(logits2, dim=1).cpu()
    ...
    #print(len(disagree_id))
    if len(disagree_id) > 1:
        update_labels = labels[disagree_id]
        update_outputs = outputs[disagree_id] 
        update_outputs2 = outputs2[disagree_id]
        loss_1, loss_2, pure_ratio_1, pure_ratio_2 = loss_coteaching(update_outputs.cuda(), 
                    update_outputs2.cuda(), update_labels, forget_rate, ind_disagree, noise_or_not)
    else:
        # print('.')
        update_labels = labels
        update_outputs = outputs.cuda()
        update_outputs2 = outputs2.cuda()
